...
const validatePin = (pin) => {
  let regex = pin.toString(10).match(/[0-9]/g);
  let validator = regex !== null ? regex.length : 0;
  return validator === 4 ? true : false;
};
...
const validateWord = (word) => {
  ...
  return (typeof(word)==='string') && !(hasNumbers.test(word)) && (validator.test(word)) ? true : false;
};

/* ------------------------------------------------------------------------------------------------
CHALLENGE 4

Write a function named hasNumber that uses a regular expression pattern to determine if a string has one or more letter followed
by one or more digit.
...
const hasNumber = (string) => {
  let validator = /\D+\d/g;
  return (typeof(string)==='string') && (validator.test(string)) ? true : false;
};
...
const validateEmail = (email) => {
  let validator = /^\w+\.?\w+@\w+\.(net|com|org)$/ ;
...
const validatePhoneNumber = (phoneNumber) => {
  let validator = /^(\((\d{3})\)[\s]?|(\d{3})[\s-]?)(\d{3})[\s-]?(\d{4})$/;
