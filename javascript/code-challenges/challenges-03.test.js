...
const containsAnd = (arr) => {
  return arr.filter(value => {
    return value.includes('and') === true;
  });
};
...
Write a function named getStatName that is an extension of your getBaseStatGreaterThan function from challenge 7. For this function, 
...
const getStatName = (arr, minBaseStat) => {
  ...
  return result.map(resultElement => {
    return resultElement.stat.name;
  });
};
...
const getCharactersWithoutChildren = (arr) => {
  return arr.filter(element => {
    return element.hasOwnProperty('children') === false;
  });
};
...
const evenOddNumericValues = (arr) => {
  let numericValuesOnly = arr.filter(value => {
    return typeof(value) === 'number';
  });
  return numericValuesOnly.map(value => {
    if (value%2 === 0) {
      return 'even';
    }
    else {
