...
Write a function named screenForNames that takes in an array of strings and uses Regex to create a new array of only
those strings that match the following rules:
...
const screenForNames = (arr) => {
  let validator = /^([MD]r|[M]rs|Ms).\s[A-Z]/g;
  let result = [];
  arr.map(string => {
    validator.test(string) && result.push(string);
  });
  return result;
}
...
Write a function named toTitleCase that takes in an array of strings and returns an array of strings with the first character in upper
...
Write a function named biggerThanLuke that, given the Star Wars data, below, returns the names of the characters whose mass
is greater than Luke's.
...
let biggerThanLuke = (arr) => {
  let findLucksMass = (arr) => {
    let mass;
    arr.forEach(obj => {
      if(obj.name === 'Luke Skywalker') {
        mass = Number(obj.mass);
      }
    });
    return mass;
  };

  let luckesMass = findLucksMass(arr);
  let results = arr.reduce((acc, char) => {
    if (Number(char.mass) > acc.luckes_mass) {
      acc.larger.push(char.name);
    }
    return acc;
  }, { luckes_mass: luckesMass, larger: [] });
  return results.larger.join(' - ');
};
...
Write a function named detectTicTacToeWin that accepts a two-dimensional array of strings. Each string is guaranteed to be either "X",
"O" or an empty string. Your function should check to see if any row, column, or either diagonal direction has three matching "X" or "O"
...
that accepts three coordinate pairs and checks the values of the array at those locations. For instance helpCheck(row1, col1, row2, col2,
row3, col3).
...
const detectTicTacToeWin = (board) => {
  ...
  for (let i=0; i<board[0].length; i++) {
    if((board[0][i] === board[1][i] && board[1][i] === board[2][i] && board[1][i] !== '') || (board[i][0] === board[i][1] && board[i][1] === board[i][2] && board[i][1] !== '')) {
      return true;
    }
  }
  return false;
