...
Write a function named addPurchases that, given an array of objects as input, uses reduce to find the total amount 
purchased. Each object contains the keys `item` and `purchasePrice` like the example.
...
const addPurchases = (arr) => {
  return arr.reduce((acc, objItem) => {
    return acc + objItem.purchasePrice;
  }, 0);
};
...
Write a function named countNumberOfElements that, given an array as input, uses reduce to count the number of elements
in the array.
...
const reversedString = (str) => {
  ...
  return strArray.reduce((acc, letter) => {
    return letter + acc;
  }, '');
};
...
const countNumberOfChildren = (arr) => {
  let numOfChildrenArray = arr.map(value => {
  ...
  return numOfChildrenArray.reduce((acc, value) => {
    return acc + value;
  }, 0);
};
...
const calculateAverage = (arr) => {
  let calc = arr.reduce((acc, value, index) => {
    return {
      count: index+1, sum: acc.sum + value,
    };
  }, {count: 0, sum: 0});
  return calc.sum / calc.count;
};
...
//   let namesArray = arr.map(value => {
//     if('name' in value.stat) {
//       return value.stat.name;
//     } else {
//       return 0;
//     }
//   });
//   return namesArray.reduce((acc, value) => {
