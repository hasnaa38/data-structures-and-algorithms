...
Write a function named count that, given an integer and an array of arrays, uses either filter, map, or reduce to count the
amount of times the integer is present in the array of arrays.
...
const count = (target, input) => {
  return input.reduce((acc, arr) => {
    let innerAcc = arr.reduce((inAcc, item) => {
      return item === target ? inAcc + 1 : inAcc;
    }, 0);
    return acc+= innerAcc;
  }, 0);
};
