...
let _quickSort = require('../quickSort');
...
describe('testing insertion sort', () => {
  it('can sort an unsorted array with just positive values and no repeats', () => {
    expect(_quickSort(testingArr1, 0, testingArr1.length-1)).toStrictEqual([ 4, 8, 15, 16, 23, 42 ]);
  });
  it('can sort an unsorted array with negative valued elements', () => {
    expect(_quickSort(testingArr2, 0, testingArr2.length-1)).toStrictEqual([ -2, 5, 8, 12, 18, 20 ]);
  });
  it('can sort and array with repeated elements', () => {
    expect(_quickSort(testingArr3, 0, testingArr3.length-1)).toStrictEqual([ 5, 5, 5, 7, 7, 12 ]);
  });
  it('can sort a nearly-sorted array', () => {
    expect(_quickSort(testingArr4, 0, testingArr4.length-1)).toStrictEqual([ 2, 3, 5, 7, 11, 13 ]);
  });
  it('returns the same array if the an array of only one element was the argument', () => {
    expect(_quickSort([2])).toStrictEqual([ 2 ]);
  });
});
